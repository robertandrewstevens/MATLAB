...
x <- 5
#0 < x < 10 # right for the wrong reason
0 < x & x < 10 # right for the wrong reason
```
...
```{python}
x = 5
print(0 < x < 10) # right for the wrong reason
```
...
print(Y[0]) # intentional error - but works
```
...
```{octave}
X = 1:5 % redefine so example runs
...
X = np.arange(1, 5 + 1, 1) # redefine X so example runs
...
Y[11] # R finds a match at i = 11 and Y = 0.891..., but MATLAB does not - why?
```
...
#X = np.linspace(0.0, 2.0*pi, num = 100)
#Y = [] # initialize
...
X = np.arange(1, 5 + 1, 1) # define so example runs
print(X)
...
```{python, eval=FALSE}
# can't get this to run in Rmd, but runs in CLI
import matplotlib.pyplot as plt
