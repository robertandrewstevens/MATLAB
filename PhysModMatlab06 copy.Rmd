...
- The first step is to define an “error function” that computes how far a given value of x is from being a solution.  In this example, the error function is
...
*If a sphere with radius r is submerged in water to a depth d, the volume of the sphere below the water line is*
    ...
    volume = (π/3)*(3*r*d^2 − d^3) as long as d < 2r
...
```{r, comment=NA}
volume <- function(rho, d, r) {
  rho - (pi/3)*(3*r*d^2 − d^3)
}
```
